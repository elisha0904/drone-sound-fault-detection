...
class CDSAgent(BaseAgent):
    ...
    def _load_datasets(self):
        ...
        audio_size = self.config.data_params.audio_size
    ...
    def _create_model(self):
        ...
        self.backbone = get_model(out_dim)

        # 사전 학습된 체크포인트 로드
        if hasattr(self.config.model_params, 'pretrained_path') and self.config.model_params.pretrained_path:
            try:
                # 체크포인트 로드
                checkpoint = torch.load(self.config.model_params.pretrained_path, map_location='cpu')
                
                # state_dict가 DataParallel로 저장된 경우 처리
                if list(checkpoint['model_state_dict'].keys())[0].startswith('module.'):
                    state_dict = checkpoint['model_state_dict']
                else:
                    state_dict = {'module.' + k: v for k, v in checkpoint['model_state_dict'].items()}
                
                # 체크포인트 로드
                self.backbone.load_state_dict(state_dict, strict=False)
                self.logger.info(f"Successfully loaded pretrained backbone from {self.config.model_params.pretrained_path}")
            except Exception as e:
                self.logger.warning(f"Failed to load pretrained backbone: {str(e)}")
                self.logger.warning("Training from scratch instead")
        else:
            self.logger.info("No pretrained backbone specified. Training from scratch")

        self.backbone = nn.DataParallel(self.backbone, device_ids=self.gpu_devices)
        ...
        if self.cls:
            ...
            cls_head = CosineClassifier(
                ...
                inc=out_dim,
                temp=self.config.loss_params.T
            )
    ...
    def _create_optimizer(self):
        ...
        # wav2vec2 parameters 추가
        wav2vec2_params = self.backbone.module.wav2vec2.parameters()
        parameters.append({
            "params": wav2vec2_params,
            "lr": lr * conv_lr_ratio if conv_lr_ratio else lr
        })

        # VGG parameters
        params_bn, _ = torchutils.split_params_by_name(self.backbone.module.vgg16, "bn")
        parameters.append({"params": params_bn, "weight_decay": 0.0})
        
        # conv layer: small lr
        _, params_conv = torchutils.split_params_by_name(self.backbone.module.vgg16, ["classifier", "bn"])
        if conv_lr_ratio:
        ...
        # classifier layer
        params_fc, _ = torchutils.split_params_by_name(self.backbone.module.vgg16, "classifier")
    ...
    def train_one_epoch(self):
        ...
        for batch_i in range(num_batches):
            ...
            # iteration over all source audios
            ...
            # iteration over all target audios
            ...
            # iteration over all labeled source audios
            ...
            for domain_name in ("source", "target"):
                ...
                if self.cls and domain_name == "source":
                    indices_lbd, audio_lbd, labels_lbd = next(source_lbd_iter)
                    indices_lbl = indices_lbd.cuda()
                    audio_lbd = audio_lbd.cuda()
                    labels_lbd = labels_lbd.cuda()
                    feat_lbd = self.model(audio_lbd)
                ...
                if (self.tgt and domain_name == "target") or self.ssl:
                    ...
                    indices_unl, audio_unl, _ = next(loader_iter)
                    audio_unl = audio_unl.cuda()
                    indices_unl = indices_unl.cuda()
                    feat_unl = self.model(audio_unl)
...
ㄹ
    ...
    def score(self, loader, name="test"):
        ...
        for batch_i, (indices, audios, labels) in enumerate(loader):
            audios = audios.cuda()
            labels = labels.cuda()

            feat = self.model(audios)
    ...
    def compute_train_features(self):
        ...
        for domain in ("source", "target"):
            ...
            for batch_i, (indices, audios, labels) in enumerate(train_loader):
